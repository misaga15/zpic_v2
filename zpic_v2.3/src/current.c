...
static inline void kernel_x( t_current* const current, const float sa, const float sb ){

    float3* restrict const J = current -> J;

    //preload and reuse registers
    float3 fl = J[-1];
    float3 f0 = J[0];
    const int nx = current -> nx;
    ...
    for( i = 0; i < nx - 1; i += 2 ) {
        ...
        fs1.x = sa * fl.x + sb * f0.x + sa * fu1.x;
        fs1.y = sa * fl.y + sb * f0.y + sa * fu1.y;
        fs1.z = sa * fl.z + sb * f0.z + sa * fu1.z;

        fs2.x = sa * f0.x + sb * fu1.x + sa * fu2.x;
        fs2.y = sa * f0.y + sb * fu1.y + sa * fu2.y;
        fs2.z = sa * f0.z + sb * fu1.z + sa * fu2.z;
...
/**
 * @brief Applies digital filtering to the current desity
 * 
...
void current_smooth( t_current* const current ) {
    ...
    // precompute filter parameters upfront
    const int xlevel = current -> smooth.xlevel;
    const int xtype = current -> smooth.xtype;
    
    if ( xtype != NONE ) {
        for( int i = 0; i < xlevel; i++) {
            kernel_x( current, 0.25f, 0.5f );  //float literals
        }
        if ( xtype == COMPENSATED ) {
            float sa = 0.25f;
	    float sb = 0.5f;
