 ...
 */
 ...
 ////
static inline void yee_b( t_emf *emf, const float dt )
{
    ...
    // 2x unrolling for better balance
    int i;
    for (i = -1; i <= nx - 1; i += 2) {
        const float3 E0 = E[i], E1 = E[i+1], E2 = E[i+2];
        
        B[i].y   += dt_dx * (E1.z - E0.z);
        B[i].z   += -dt_dx * (E1.y - E0.y);
        
        B[i+1].y += dt_dx * (E2.z - E1.z);
        B[i+1].z += -dt_dx * (E2.y - E1.y);
    }
...
static inline void yee_e( t_emf *emf, const t_current *current, const float dt )
{
    const float dt_dx = dt / emf->dx;
    const float neg_dt = -dt;
    ...
    int i;
    for (i = 0; i <= nx - 1; i += 2) {
        const float3 B0 = B[i-1], B1 = B[i], B2 = B[i+1];
        const float3 J0 = J[i], J1 = J[i+1];
        
        E[i].x   += neg_dt * J0.x;
        E[i].y   += -dt_dx * (B1.z - B0.z) + neg_dt * J0.y;
        E[i].z   += dt_dx * (B1.y - B0.y) + neg_dt * J0.z;
        
        E[i+1].x += neg_dt * J1.x;
        E[i+1].y += -dt_dx * (B2.z - B1.z) + neg_dt * J1.y;
        E[i+1].z += dt_dx * (B2.y - B1.y) + neg_dt * J1.z;
    }
    
    // Handle remaining elements
    E[i].x += neg_dt * J[i].x;
    E[i].y += -dt_dx * (B[i].z - B[i-1].z) + neg_dt * J[i].y;
    E[i].z += dt_dx * (B[i].y - B[i-1].y) + neg_dt * J[i].z;
}
