...
void spec_set_u( t_species* spec, const int start, const int end )
{
 ...
 //    * Version 1 momentum initialization
    ...
    const float uth_x = spec->uth[0];
    const float uth_y = spec->uth[1]; 
    const float uth_z = spec->uth[2];
    const float ufl_x = spec->ufl[0];
    const float ufl_y = spec->ufl[1];
    const float ufl_z = spec->ufl[2];

    // Single allocation for cell data
    float3 * restrict net_u = (float3 *) malloc( nx * sizeof(float3) );
    int * restrict    npc   = (int *) malloc( nx * sizeof(int) );

    // Zero momentum grids
    memset(net_u, 0, nx * sizeof(float3));
    memset(npc,   0, nx * sizeof(int));

    // SINGLE COMBINED LOOP: Initialize thermal + accumulate momentum
    for (int i = start; i <= end; i++) {
        // Initialize thermal component
        float rn = rand_norm();
        spec->part[i].ux = uth_x * rn;
        spec->part[i].uy = uth_y * rn;
        spec->part[i].uz = uth_z * rn;
    ...
    // Precompute reciprocals and normalize
    for (int i = 0; i < nx; i++) {
        if (npc[i] > 0) {
            const float inv_npc = 1.0f / npc[i];
            net_u[i].x *= inv_npc;
            net_u[i].y *= inv_npc;
            net_u[i].z *= inv_npc;
        }
    ...
    // SECOND LOOP: Subtract average momentum and add fluid component
    for (int i = start; i <= end; i++) {
        const int idx = spec->part[i].ix;
        spec->part[i].ux += ufl_x - net_u[idx].x;
        spec->part[i].uy += ufl_y - net_u[idx].y;
        spec->part[i].uz += ufl_z - net_u[idx].z;
    }
 ...
 * @param current   Electric current density
 */
static inline void dep_current_zamb(
    int ix0, int di,
    float x0, float dx,
    float qnx, float qvy, float qvz,
    t_current *current )
{
    // Virtual particle structure
    typedef struct {
    ...
    // --- Particle Splitting Logic ---
    vp[0].x0 = x0;
    vp[0].dx = dx;
    vp[0].x1 = x0 + dx;
    vp[0].qvy = qvy * 0.5f;  // Use float literal
    vp[0].qvz = qvz * 0.5f;
    vp[0].ix = ix0;

    // Handle particle crossing cell boundaries
    if ( di != 0 ) {
        int ib = ( di == 1 );  // Simplified boundary calculation
    ...
    // --- Optimized Current Deposition ---
    float3* restrict const J = current->J;

    // Handle most common case (single virtual particle) without loop overhead
    if (vnp == 1) {
        const float x0_val = vp[0].x0;
        const float x1_val = vp[0].x1;
        const int ix = vp[0].ix;

        // Optimized shape factor calculation: (3a + b)/2 instead of (a+b) + (a-b)/2
        const float Sy0 = (3.0f * (1.0f - x0_val) + (1.0f - x1_val)) * 0.5f;
        const float Sy1 = (3.0f * x0_val + x1_val) * 0.5f;

        // Load both affected grid points into registers
        float3 j0 = J[ix];
        float3 j1 = J[ix + 1];

        // Accumulate currents using precomputed values
        j0.x += qnx * vp[0].dx;
        j0.y += vp[0].qvy * Sy0;
        j1.y += vp[0].qvy * Sy1;
        j0.z += vp[0].qvz * Sy0;  // Reuse shape factors for z-component
        j1.z += vp[0].qvz * Sy1;
        ...
        // Handle two virtual particles with minimal overhead
        for (int k = 0; k < 2; k++) {
            const float x0_val = vp[k].x0;
            const float x1_val = vp[k].x1;
            const int ix = vp[k].ix;
            ...
            J[ix].x     += qnx * vp[k].dx;
            J[ix].y     += vp[k].qvy * Sy0;
            J[ix + 1].y += vp[k].qvy * Sy1;
            J[ix].z     += vp[k].qvz * Sy0;
            J[ix + 1].z += vp[k].qvz * Sy1;
        }
    }
    else {
        // General case for rare vnp > 2 situations
        for (int k = 0; k < vnp; k++) {
            const float x0_val = vp[k].x0;
            const float x1_val = vp[k].x1;
            const int ix = vp[k].ix;
 ...
 */
static inline void interpolate_fld( const float3* restrict const E, const float3* restrict const B,
              const t_part* restrict const part, float3* restrict const Ep, float3* restrict const Bp )
{
    const int i = part->ix;
    const float w1 = part->x;
    
    // Precompute all values
    const float om_w1 = 1.0f - w1;
    const int mask = -(w1 < 0.5f);           // -1 if w1<0.5, else 0
    const int ih = i + mask;                  // i-1 if w1<0.5, else i
    const float w1h = w1 - (0.5f * (mask | 1));  // w1+0.5 if w1<0.5, else w1-0.5
    const float om_w1h = 1.0f - w1h;

    // Load field values once
    const float3 Ei = E[i], Ei1 = E[i+1];
    const float3 Eih = E[ih], Eih1 = E[ih+1];
    const float3 Bi = B[i], Bi1 = B[i+1];
    const float3 Bih = B[ih], Bih1 = B[ih+1];

    // Interpolate electric field
    Ep->x = Eih.x * om_w1h + Eih1.x * w1h;
    Ep->y = Ei.y * om_w1 + Ei1.y * w1;
    Ep->z = Ei.z * om_w1 + Ei1.z * w1;

    // Interpolate magnetic field
    Bp->x = Bi.x * om_w1 + Bi1.x * w1;
    Bp->y = Bih.y * om_w1h + Bih1.y * w1h;
    Bp->z = Bih.z * om_w1h + Bih1.z * w1h;
}
